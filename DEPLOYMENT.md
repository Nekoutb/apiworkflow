...
|---|---|---|
| `APP_DATABASE_URL` | Optional | Postgres URL override. Leave **unset** to use the local Postgres provisioned by `vps-bootstrap.sh` (the supported default) |
...
**About the initial data:** the first deploy onto an empty database automatically imports `scripts/initial-seed.sql` (1410 lines, ~41 KB) — schema + reference data + admin users carried over from the legacy environment. This happens once. On every subsequent deploy the script detects existing tables and skips the import.
...
To override either value (e.g. point at an external managed Postgres, or rotate AUTH_SECRET), set the corresponding `APP_*` GitHub Secret. The workflow logs which source it used on every deploy:
```
