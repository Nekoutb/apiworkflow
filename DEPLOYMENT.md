...
### 3. No app secrets needed in GitHub

**Zero `APP_*` secrets are required.** The deploy script (`scripts/vps-deploy.sh`) self-heals on every run:

- **First deploy on a fresh VPS**: generates `AUTH_SECRET` and the DB password on the VPS itself, creates the PostgreSQL role + database, imports `scripts/initial-seed.sql`, writes `.env.production` locally.
- **Subsequent deploys**: reuses the stored values, skips the seed (detects existing tables), just rebuilds `.env.production` with the same secrets to keep the file in sync with any changed runtime config (port, domain, etc.).

Persistent state on the VPS (mode 600, owned by your SSH user, never rotated):
- `/var/www/cmipaportal/shared/.auth-secret`
- `/var/www/cmipaportal/shared/.db-password`

**About the database:** `vps-bootstrap.sh` installs PostgreSQL 16. The deploy script creates the `cmipaportal` database + user on first run. Nightly `pg_dump` backups go to `/var/backups/cmipaportal/` with 7-day retention.

**About the initial data:** the first deploy onto an empty database automatically imports `scripts/initial-seed.sql` (1410 lines, ~41 KB) — schema + reference data + admin users. This happens once. On every subsequent deploy the script detects existing tables and skips the import.

To regenerate the seed from a current production snapshot, run `scripts/neon-backup.ps1` (or `pg_dump` on the VPS) and copy the output over `scripts/initial-seed.sql`. New first-time installs will then start from that newer baseline.

**Optional integrations** (Anthropic Claude, Resend, Vercel Blob) default to empty stubs. To add API keys later, SSH in and edit `/var/www/cmipaportal/shared/.env.production` directly — the deploy script preserves any values you don't manage (well, currently it overwrites the file; to keep manual edits, add an `extras.env` source — covered in B14 / Claude integration).
...
| **Self-heal env** | Deploy script on VPS generates `AUTH_SECRET` (if missing), provisions DB user (if missing), writes `shared/.env.production` from local state | Hard fail only if PostgreSQL isn't installed |
| **Env link** | Symlink `shared/.env.production` into release | (won't fail — sync step composed it) |
| **Dependencies** | `npm ci --omit=dev` OR hardlink reuse from previous release | Rollback to previous release |
| **Seed if empty** | If local Postgres has no user tables, import `scripts/initial-seed.sql` (schema + reference data + admin users). Skipped on subsequent deploys | Rollback |
| **Prisma db push** | Schema sync to local Postgres (idempotent, refuses data loss). No-op right after seed import | Rollback |
...
| `Missing /var/www/cmipaportal/shared/.env.production` | Can't happen with current deploy script — it composes the file itself. If you see this, you're on an older release of `vps-deploy.sh`; trigger a fresh deploy from the latest `main` |
| `PostgreSQL is not installed` | Run `vps-bootstrap.sh` on the VPS first (it installs PostgreSQL + grants the SSH user scoped `sudo -u postgres` so the deploy script can create the DB) |
