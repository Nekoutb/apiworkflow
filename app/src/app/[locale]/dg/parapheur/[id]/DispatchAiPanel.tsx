...
function Inner({
  state, documentId, aiEnabled, onAnalyze, onReanalyze, pending, isEn,
}: {
  state: State;
  documentId: string;
  aiEnabled: boolean;
    ...
    return (
      <div className="border border-line bg-bgsoft p-5">
        <div className="mb-2 text-[10.5px] font-bold uppercase tracking-[0.18em] text-ink-3">
          {isEn ? 'Dispatch suggestion' : 'Suggestion de dispatch'}
        </div>
        <h3 className="serif text-[16px] font-semibold text-ink">{isEn ? 'Manual mode' : 'Mode manuel'}</h3>
        <p className="serif mt-2 text-[12.5px] italic text-ink-3">
          {isEn
            ? 'Automatic analysis is not active on this server. Choose the target unit manually below.'
            : "L'analyse automatique n'est pas active sur ce serveur. Choisissez l'unité cible manuellement ci-dessous."}
        </p>
        <button
          type="button"
          onClick={onAnalyze}
        ...
        >
          {pending ? '…' : (isEn ? 'Run analysis' : "Lancer l'analyse")}
        </button>
      </div>
    );
  }
      ...
      <div className={`relative overflow-hidden border border-obsidian ${dark} p-6 text-white`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <h3 className="serif text-[16px] font-semibold">
            {isEn ? 'Document being analysed…' : 'Document en cours d\'analyse…'}
          </h3>
        </div>
        <p className="serif mt-3 text-[12.5px] italic text-white/55">
          {isEn ? 'Usually a few seconds for a single-page letter.' : 'Habituellement quelques secondes pour un courrier d\'une page.'}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="border border-cmred bg-cmred-50 p-5">
        <div className="mb-2 text-[10.5px] font-bold uppercase tracking-[0.18em] text-cmred">
          {isEn ? '⚠ Analysis error' : "⚠ Erreur d'analyse"}
        </div>
  ...
  if (state.kind !== 'cached' && state.kind !== 'fresh') {
    return (
      <div className="border border-line bg-white p-5 text-[12.5px] italic text-ink-3">
        {isEn ? 'Analysis unavailable.' : 'Analyse non disponible.'}
      </div>
    );
  }
  const s = state.s;
  ...
  const modeLabel = isCached
    ? `${isEn ? 'cache' : 'cache'} · ${at.toLocaleString(dtLocale, { dateStyle: 'short', timeStyle: 'short' })}`
    : state.mode === 'stub'
    ? (isEn ? 'demo mode (stub)' : 'mode démo (stub)')
    : (isEn ? 'analysed just now' : 'analysé à l\'instant');
  ...
  const confidenceLabel =
    s.confidence === 'high'   ? (isEn ? 'High' : 'Élevée') :
    s.confidence === 'medium' ? (isEn ? 'Medium' : 'Moyenne') :
                                (isEn ? 'Low' : 'Faible');
      ...
      <div className={`relative overflow-hidden border border-obsidian ${dark} p-6 text-white`}>
        ...
        <div className="mt-6 border-t border-white/15 pt-5">
          <div className="text-[10.5px] font-bold uppercase tracking-[0.18em] text-gold-500">
            {isEn ? 'Suggested unit' : 'Unité suggérée'}
          </div>
          <div className="mt-2 text-[18px] font-bold leading-tight text-white">
            {roleLabel(s.suggestedRole, localeShort)}
          </div>
        ...
        {s.alternatives && s.alternatives.length > 0 && (
          <div className="mt-5 border-t border-white/15 pt-5">
            <div className="text-[10.5px] font-bold uppercase tracking-[0.18em] text-gold-500">
              {isEn ? 'Alternatives' : 'Alternatives'}
            </div>
            <ul className="mt-2 space-y-1 text-[12px]">
              {s.alternatives.map((r: StaffRole) => (
                <li key={r} className="flex justify-between gap-3">
                  <span className="text-white">{roleLabel(r, localeShort)}</span>
        ...
        <div className="mt-5 flex items-center justify-between border-t border-white/10 pt-3 text-[10.5px] text-white/45">
          ...
          >
            {pending ? '…' : (isEn ? '↻ Re-analyse' : '↻ Re-analyser')}
          </button>
        </div>
      </div>
...
function DispatchForm({
  ...
  const effectiveLabel = roleLabel(effectiveRole, localeShort);
    ...
    return (
      <div className="border-2 border-cmgreen-800 bg-cmgreen-50 p-5">
        <div className="text-[10.5px] font-bold uppercase tracking-[0.18em] text-cmgreen-900">
          {isEn ? '✓ Document dispatched' : '✓ Document dispatché'}
        </div>
        <h3 className="serif mt-2 text-[16px] font-bold text-ink">
          {isEn ? 'Forwarded to ' : 'Transmis à '}{state.targetLabel ?? effectiveLabel}
        </h3>
        <p className="serif mt-2 text-[12.5px] italic text-cmgreen-900/80">
          {isEn
            ? 'The document was transferred via the Mail Service to the target unit. It now appears in that unit\'s folder, where the unit head can take it up or return it.'
            : "Le document a été transféré via le Service du Courrier vers l'unité cible. Il apparaît désormais dans le parapheur de cette unité, où le chef d'unité peut le prendre en charge ou le renvoyer."}
        </p>
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          >
            {isEn ? 'View unit folder →' : 'Voir le parapheur unité →'}
          </a>
        </div>
      </div>
    );
    ...
    <form action={formAction} className="border border-line bg-white">
      <div className="border-b border-line bg-bgsoft px-4 py-3">
        <div className="text-[10.5px] font-bold uppercase tracking-[0.18em] text-gold-700">
          {isEn ? '⬇ Dispatch' : '⬇ Dispatcher'}
        </div>
        <h3 className="serif mt-0.5 text-[15px] font-semibold text-ink">
          {isEn ? 'Forward to the responsible unit' : "Transmettre vers l'unité responsable"}
        </h3>
      </div>
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="flex gap-1">
          ...
          >
            {isEn ? '✓ Accept the suggestion' : '✓ Accepter la suggestion'}
          </button>
          <button
            type="button"
          ...
          >
            {isEn ? '✎ Choose another unit' : '✎ Choisir une autre unité'}
          </button>
        </div>
        ...
        {overrideMode && (
          <div>
            ...
            >
              {isEn ? 'Target unit (org chart · 37 roles)' : 'Unité cible (organigramme · 37 rôles)'}
            </label>
            <select
              id="role-override"
              value={picked}
            ...
            {alternatives.length > 0 && (
              <div className="mt-2 text-[10.5px] text-ink-3">
                {isEn ? 'Suggested alternatives:' : 'Alternatives proposées :'}{' '}
                {alternatives.map((r, i) => (
                  ...
                  >
                    {roleLabel(r, localeShort)}{i < alternatives.length - 1 ? ', ' : ''}
                  </button>
                ))}
              </div>
            )}
        ...
        <div>
          <label
            ...
            className="mb-1.5 block text-[10.5px] font-semibold uppercase tracking-[0.18em] text-ink-2"
          >
            {isEn ? 'GM instructions (optional)' : 'Instructions du DG (optionnel)'}
          </label>
          <textarea
            id="instructions"
            name="instructions"
            rows={3}
            maxLength={2000}
            placeholder={isEn ? 'e.g. Treat as priority · 10-day legal deadline.' : 'ex. À traiter en priorité · délai légal 10 jours.'}
          ...
          <p className="mt-1 text-[10.5px] italic text-ink-4">
            {isEn ? 'Shown as an internal note to the receiving unit head.' : "Affichées comme note interne pour le chef d'unité destinataire."}
          </p>
        </div>
        ...
        >
          {pending ? (isEn ? 'Dispatching…' : 'Dispatch en cours…') : (isEn ? `Dispatch to ${effectiveLabel} →` : `Dispatcher vers ${effectiveLabel} →`)}
        </button>

        <p className="text-[10.5px] italic text-ink-4">
          {isEn
            ? 'Transits via the Mail Service. Document status: AWAITING_DG_ANALYSIS → ASSIGNED.'
            : 'Transit via le Service du Courrier (règle B14.5). Statut du document : AWAITING_DG_ANALYSIS → ASSIGNED.'}
        </p>
      </div>
    </form>
  );
